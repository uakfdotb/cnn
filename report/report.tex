...
\begin{abstract}
In this project, we implement a CPU-based convolutional neural network (CNN) library in Python, using the \texttt{numpy} library for fast array operations. The implementation process involves writing codes to construct the complex structure of the neural net, perform forward propagation and pooling, and perform backward propagation using Stochastic Gradient Descent. Besides the implementation, we also evaluate CNN performance in terms of classification accuracy and training time on the MNIST database under a wide range of network parameters, and compare CNN to feature extraction approaches such as HOG. We find that CNN is able to achieve much lower misclassification rate (as low as 1.3\%), but requires significantly more time for training of features.
\end{abstract}
...
\section{Implementation, Experimental setup And Data Set} \label{sec:evalcnn}

We implement a CPU-based convolutional neural network library \cite{git:cnn} in Python using the \texttt{numpy} package, which provides fast scientific computing operations. In Python, we define a network class which has attributes that represent the number of layers and the number of neurons in each layer. Our code can perform either "softmax" or "sigmoid" function for the output of each neuron. Our code initializes all the weights to be random and uniformly distributed within [0,1), which serves the purpose of symmetric breaking. The network class class has a "forward propagation" function and a "backward propagation" function. The "forward propagation" function just involves looping over all the neurons, combining the output of their values to the neurons in the final output layer. In backward propagation, our code first averages the derivatives across a subset of the data (Stochastic Gradient Descent) and this is the step that is most computationally expensive. After that, we update the weights simultaneously by applying the momentum formula in section \ref{subsec:SGD}. Please see appendix for more details.

We also evaluate our neural net's performance on the MNIST dataset of handwritten digits in terms of overall training algorithm, network structure, number of filters, size of filters, number of pooling regions, and output layer activation function.

The Mixed National Institute of Standards and Technology (MNIST) dataset contains 60,000 training images and 10,000 testing images. Its name comes from the NIST dataset that it is an adaptation of. In the NIST dataset, training and testing images were taken from two different sources, while in MNIST, the sources are merged. Each MNIST image contains a single handwritten digit (from zero to nine). The images are all 28 pixels by 28 pixels and grayscale.

For our experimentation, we use a basic default setting of the neural network, and then adjust parameters of it to identify patterns in the change in performance, such as test error rate and average time taken to run an iteration. Specifically, we use a default setting of 64 filters, each 13 by 13, with a pooling layer of size 2 by 2. The output from the pooling stage, then, consists of 256 values, which are fully connected to a final output layer consisting of ten neurons (without additional hidden layers in between).

\section{CNN Evaluation}

In this section, we show and analyze CNN performance under various network parameters tested in our experiment.
...
\hline
No hidden layer & 0.0269 & 30.48 \\
\hline
10-neuron hidden layer & 0.0173 & 30.80 \\
\hline
16-neuron hidden layer & 0.0174 & 30.66 \\
\hline
32-neuron hidden layer & 0.0181 & 30.56 \\
\hline
